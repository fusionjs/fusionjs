...
test('ssr with accept header', async () => {
  ...
  expect(!(ctx.body as string).includes(element)).toBeTruthy();
...
test('HTML escaping works', async () => {
  ...
  const ctx = await run(app);
  expect(
    (ctx.body as string).includes('<html lang="\\u0022\\u003E">')
  ).toBeTruthy();
  expect(
    (ctx.body as string).includes('<body test="\\u0022\\u003E">')
  ).toBeTruthy();
  expect(
    (ctx.body as string).includes('<title>\\u003C/title\\u003E</title>')
  ).toBeTruthy();
});
...
test('head and body must be sanitized', async () => {
  ...
  const ctx = await run(app);
  expect(
    (ctx.body as string).includes('<meta charset="\\u0022\\u003E" />')
  ).toBeTruthy();
  expect(
    (ctx.body as string).includes('<div>\\u0022\\u003E</div>')
  ).toBeTruthy();
});
