...
import type {FusionPlugin, Token} from '../types';
...
const TokenEAsNullable: Token<EType | undefined | null> =
  createToken('TokenEAsNullable');
...
test('dependency registration', () => {
  ...
  const PluginA: FusionPlugin<undefined, AType> = createPlugin({
    provides: () => {
      counters.a++;
...
test('retrieve dependency', () => {
  const app = new App('el', (el) => el);
  const TokenA = createToken<{a: string}>('a');
  const PluginA = createPlugin({
    provides: () => {
      return {
        a: 'Hello',
