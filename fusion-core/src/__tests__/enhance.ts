...
test('enhancement', (done) => {
  ...
  const BaseFn: FusionPlugin<undefined, FnType> = createPlugin({
    provides: () => {
      return (arg) => arg;
    },
  });
...
test('enhancement with a plugin', (done) => {
  ...
  const BaseFnEnhancer = (fn: FnType): FusionPlugin<undefined, FnType> => {
    return createPlugin({
      provides: () => {
        return (arg) => {
          return fn(arg) + ' enhanced';
...
test.skip('Plugin enhancer on unregistered token with no dependencies', (t) => {
  ...
  type FnType = (a: string) => string;
  const FnToken: Token<FnType> = createToken('FnType');
  let BaseFnEnhancer = (fn: FnType): FusionPlugin<undefined, FnType> => {
    return createPlugin({
      provides: () => {
        return (arg) => {
          return fn(arg) + ' enhanced';
