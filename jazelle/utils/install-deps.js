...
const installDeps = async (root, deps, hooks = {}) => {
  ...
  await Promise.all(
    deps.map(async dep => {
      ...
      hoisted = await populate(cacheDir, deps, dep, 'dependencies', object, hoisted);
      hoisted = await populate(cacheDir, deps, dep, 'devDependencies', object, hoisted);
      if (!isCached) {
...
async function populateTransitive(root, dep, hoisted) {
  ...
  const deps = (await exec('find . -name package.json -type f', {cwd: cacheDir}))
    .trim()
    .split('\n')
  ...
  for (let i = 0, size = 300; i < deps.length; i += size) {
    await Promise.all(
      deps.slice(i, i + size).map(async dep => {
        ...
        await exec(`${prep} rm -rf ${dep} && ln -s ${cacheDir}/${dep} ${dep}`, {cwd: dir});
      })
    )
  }
}
