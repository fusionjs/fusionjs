...
*/
const setupSymlinks /*: SetupSymlinks */ = async ({root, deps}) => {
  ...
  await Promise.all(
    deps.map(async dep => {
      ...
      if (!(await exists(`${modulesDir}/${dep.meta.name}`))) {
        ...
        await spawn('ln', ['-sFfn', dep.dir, basename], {
          cwd: `${modulesDir}/${ns}`,
        });
      }
      ...
      if (!(await exists(`${dep.dir}/node_modules/.bin`))) {
        ...
        await spawn('ln', ['-sFfn', `${modulesDir}/.bin`, '.bin'], {
      ...
      for (const cmd in bin) {
        if (!(await exists(`${modulesDir}/.bin/${cmd}`))) {
          await spawn('ln', ['-sFfn', `${dep.dir}/${bin[cmd]}`, cmd], {
            cwd: `${modulesDir}/.bin`,
          });
        }
      }
